...
public class MyBirth {
    ...
    /*
     ...
     2. 메서드 내부에 데이터 유효성 검증 로직을 작성하여
      적절한 데이터만 멤버변수(필드)에 저장시키고, 접근제한자는
      public으로 설정하여 외부에서 누구나 사용할 수 있게 합니다.

     3. 메서드 이름은 일반적으로 set + 필드이름 으로 설정합니다. (관례)
     */
    ...
    /*
     ...
     2. setter와 마찬가지로 public제한을 통해 외부에 메서드를 공개하고,
      이름은 일반적으로 get + 멤버변수(필드)이름 으로 지정합니다.
     */
    ...
    public void setDay(int day) {
        if (day > 31 || day < 1) {
            ...
            if (this.month == 0) {
                System.out.println("일자를 입력하기 위해서는 월 입력이 필요합니다.");
                return;
            }
    ...
    private boolean isValidateMonth(int day) {
        switch (this.month) {
            ...
            default:
                return true;   // 이미 1~31 사이에 정수라는걸 위에서 검증하고 넘어왔기 때문에 true로 빠짐
        }

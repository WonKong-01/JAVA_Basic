...
//        else return true;
        ...
        return indexOf(searchTarget) != -1;  // <- 한줄로 요약 가능
    }
    ...
    static boolean isOutOfBounds(int targetIndex) {
        return targetIndex > foods.length - 1 || targetIndex < 0;     // <- 한줄로 작성가능
    }
...
//    }
    ...
    static void insert(int targetIndex, String newFoodName) {
        if (isOutOfBounds(targetIndex)) return;   // <- 한줄로 요약 가능
    ...
    // size에 따라 크기가 큰 or 크기가 작은 배열 리턴
